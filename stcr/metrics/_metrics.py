...
def get_largest_clonotypes(adata, n=20):
    df = adata.obs[[adata.uns["stcr_clone_key"],"clone_size"]]
    return df.sort_values("clone_size",ascending=False)[adata.uns['stcr_clone_key']].unique().tolist()[:n]
...
def phenotypic_entropies(adata, method="probabilistic", normalized=True, decimals=5):
    joint_distribution(adata, method=method)
    tcr_sequences = adata.obs[adata.uns["stcr_clone_key"]].tolist()
...
def clonotypic_entropies(adata, method="probabilistic", normalized=True, base=2, decimals=5):
    unique_phenotypes = adata.uns["stcr_unique_phenotypes"]
...
def clonality(adata):
    phenotypes = adata.obs[adata.uns["stcr_phenotype_key"]].tolist()
    unique_phenotypes = np.unique(phenotypes)
    entropys = dict()
    df = adata.obs.copy()
    for phenotype in unique_phenotypes:
        pheno_df = df[df[adata.uns['stcr_phenotype_key']]==phenotype]
        clonotypes = pheno_df[adata.uns["stcr_clone_key"]].tolist()
        unique_clonotypes = np.unique(clonotypes)
        nums = []
        for tcr in unique_clonotypes:
            tcrs = pheno_df[pheno_df[adata.uns["stcr_clone_key"]]==tcr]
...
def clone_fraction(adata, groupby):
    ...
    for group in set(adata.obs[groupby]):
        ...
        clones = sdata.obs[sdata.uns["stcr_clone_key"]].tolist()
        for c in set(clones):
...
def marginal_phenotypic(adata, clones=None, probability=False, method="probabilistic"):
    joint_distribution(adata,method=method)
    if clones == None:
        clones = adata.obs[adata.uns["stcr_clone_key"]].tolist()
...
def flux(adata, key, from_this, to_that, clones=None, method="probabilistic", distance_metric="l1"):
    ...
    this_clones = set(this.obs[this.uns["stcr_clone_key"]])
    that_clones = set(that.obs[this.uns["stcr_clone_key"]])
