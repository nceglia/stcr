...
def joint_distribution(adata, method='probabilistic'):
    if method == "probabilistic":
        ...
        matrix["clonotype"] = adata.obs[adata.uns["stcr_clone_key"]]
        ...
        unique_tcrs = adata.uns["stcr_unique_clonotypes"]
...
def register_phenotype_key(adata, phenotype_key, order=None):
    ...
    if order==None:
        adata.uns["stcr_unique_phenotypes"] = np.unique(adata.obs[phenotype_key].tolist())
    adata.uns["stcr_phenotype_key"] = phenotype_key

def register_clonotype_key(adata, tcr_key):
    assert tcr_key in adata.obs, "Key {} not found.".format(tcr_key)
    adata.uns["stcr_clone_key"] = tcr_key
    adata.uns["stcr_unique_clonotypes"] = np.unique(adata.obs[tcr_key].tolist())
...
def clone_size(adata, key_added="clone_size", return_counts=False):
    tcr_key = adata.uns["stcr_clone_key"]
