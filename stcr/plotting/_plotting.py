...
sns.set_palette(sns.color_palette(stcr_colors))
...
def phenotypic_flux(adata, splitby, order, clones=None, normalize=True, nt=True, method="probabilistic", phenotype_colors=None, save=None, figsize=(6,3), show_legend=True):
    ...
    phenotypes = Phenotypes(adata.obs[adata.uns["stcr_phenotype_key"]].unique())
    ...
    for bc, condition, seq, phenotype in zip(adata.obs.index,
                                         adata.obs[splitby],
                                         adata.obs[adata.uns["stcr_clone_key"]],
                                         adata.obs[adata.uns["stcr_phenotype_key"]]):
    ...
    if phenotype_colors==None:
        phenotype_colors = dict(zip(set(adata.obs[adata.uns["stcr_phenotype_key"]]), stcr_colors))
...
def probability_ternary(adata, phenotype_names, splitby, conditions, method="probabilistic", nt=False, top_n=None, scale_function=None,color="k",save=None):
    ...
    if top_n == None:
        top_n = len(set(adata.obs[adata.uns["stcr_clone_key"]]))
...
def top_clone_umap(adata, reduction="umap", top_n=10, fg_alpha=0.9, fg_size=25, bg_size=0.1, bg_alpha=0.6, figsize=(12,5), return_df=False,save=None):
    df = adata.obs
    seq_column = adata.uns["stcr_clone_key"]
    ...
    colors = stcr_colors + stcr_colors + stcr_colors
    order = []
...
def stcr_boxplot(adata, function, groupby=None,ylabel="", splitby=None,figsize=(8,4),s=20,order=None, palette=None, rotation=0):
    if palette == None:
        palette = stcr_colors
...
def clonality(adata, groupby = None, splitby=None, s=10, order=None, figsize=(12,5), palette=None, rotation=0):
    return stcr_boxplot(adata,clonality_tl, ylabel="Clonality", groupby=groupby, splitby=splitby, s=s, figsize=figsize, order=order, palette=palette, rotation=rotation)

def clonotypic_entropy(adata, method="probabilistic", normalized=True, groupby=None, splitby=None, s=10, figsize=(12,5), order=None, palette=None, rotation=0):
    func = lambda x : centropies(x, normalized=normalized, method=method)
    return stcr_boxplot(adata, func, groupby=groupby, ylabel="Clonotypic Entropy", splitby=splitby, s=s, figsize=figsize, order=order, palette=palette, rotation=rotation)
...
def set_color_palette(adata, columns):
    ...
    main_color_map = dict()
    adata = adata.copy()
    colors = stcr_colors.copy() + stcr_colors.copy() + stcr_colors.copy()
    for x in columns:
        ct = []
...
def flux(adata, key, order, groupby, paint_dict=None, method="probabilistic", paint=None, distance_metric="l1", figsize=(12,5), paint_order=None, palette=None):
    ...
    if paint != None:
        ...
        if paint_dict != None:
            ...
            pcolors = dict(zip(paint_categories, stcr_colors))
...
def polar_plot(adata, phenotypes=None, statistic="entropy", method="probabilistic", save=None, figsize=(6,6), title=None, alpha=0.6, fontsize=15, splitby=None, bbox_to_anchor=(1.15,1.), linewidth=5., legend_fontsize=15, color_dict=None):
    ...
    subset = adata[adata.obs[adata.uns["stcr_phenotype_key"]].isin(phenotypes)]
    for i, split in enumerate(splits): 
        if color_dict == None:
            colorx = stcr_colors[i]
        else:
